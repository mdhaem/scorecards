<?php
  ...
  class CardGame
  {
	  ...
	  public static function saveNewGroupName($groupName){
  		...
  		$sql = "INSERT INTO iscorecards.GroupName (groupName) VALUES ( $groupName)";
  		if (!mysql_query($sql))
  		{
	  ...
	  public static function saveNewPlayerGroup($idPlayer, $idCardGame, $playerGroup){
  		...
  		$sql = "INSERT INTO iscorecards.PlayerGroup (idPlayer, idCardGame, playerGroup) VALUES ( $idPlayer, $idCardGame, $playerGroup)";
  		if (!mysql_query($sql))
  		{
  		  	return die('Insert Error: ' . mysql_error());
  		}
	  ...
	  public static function saveNewPlayer($playerName){
		...
		$sql = "INSERT INTO iscorecards.Player (playerName) VALUES ( $playerName)";
		if (!mysql_query($sql))
		{
		  	return die('Insert Error: ' . mysql_error());
		}
     ...
     public static function getMaxPlayerGroup()
     {
		...
		$sql = 'SELECT MAX(playerGroup) AS playerGroup FROM iscorecards.PlayerGroup';
		$result_array = array();
		$result = mysql_query($sql,$con);
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
     ...
     public static function getNextPlayerGroup()
     {
		...
		$sql = 'SELECT MAX(playerGroup)+1 AS playerGroup FROM iscorecards.PlayerGroup';
		$result_array = array();
		$result = mysql_query($sql,$con);
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    			$row_array['playerGroup'] = $row['playerGroup'];
